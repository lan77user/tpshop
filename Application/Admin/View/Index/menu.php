...
<?php foreach ($power_info_father as $k => $v):?>
    <div id="menu-list">
        <ul id="menu-ul">
            ...
            {$v.power_name}
                <?php foreach ($power_info_son as $key => $val):?><?php if ($val['power_pid'] == $v['power_id']) :?>
                <ul>
                    <li class="menu-item"><a href="__MODULE__/{$val.power_c}/{$val.power_a}" target="main-frame">{$val.power_name}</a></li>
                </ul><?php endif;?>
                <?php endforeach;?>
            </li>
        </ul>
